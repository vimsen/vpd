...
function addToChart(divId, pieDivId, object,prosumptionType, dateFrom, dateTo,titleText) {
  ...
  $.each(object, function(index, element) {
    ...
    var pieObj = {name:index+"@"+element[0].Prosumers, y: yPie };
...
*/
    ...
    if(index.toLowerCase().search(prosumptionType) != -1  && index.toLowerCase().search("power") == -1 &&index.toLowerCase().search("production") == -1){
     ...
     // chart2.showLoading();
    //change title
    chart2.setTitle({ text: 'Historical ' +titleText+' STATE per Device for all VGWs from '+moment(dateFrom).format('DD/MM/YYYY') +' to '+moment(dateTo).format('DD/MM/YYYY')});
